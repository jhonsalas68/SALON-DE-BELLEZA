...
@section('title', 'Editar Producto - Salon Anita')

@section('header')
    <div>
        <h2 class="text-3xl font-extrabold text-gray-800 tracking-tight">Editar Producto</h2>
        <p class="text-gray-500 font-medium">Actualiza la información de {{ $producto->nombre }}.</p>
    </div>
@endsection
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <div class="flex items-center space-x-6">
                        <div class="w-24 h-24 bg-rose-50 border-2 border-dashed border-rose-200 rounded-3xl flex items-center justify-center overflow-hidden" id="preview-container">
                            @if($producto->imagen)
                                <img src="{{ asset($producto->imagen) }}" class="w-full h-full object-cover">
                            @else
                                <i class="fas fa-image text-3xl text-rose-300"></i>
                            @endif
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-[9px] text-gray-400 mt-2 italic">Deja vacío para mantener la imagen actual. Máximo 2MB.</p>
                        </div>
                    </div>
                ...
                <div class="md:col-span-1 space-y-2">
                    ...
                    <select name="promotor_id" 
                        class="w-full bg-rose-50/30 border border-rose-100 rounded-2xl p-4 text-gray-800 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all appearance-none cursor-pointer font-bold">
                        <option value="">-- Sin Promotor Relacionado --</option>
                        @foreach($promotores as $promotor)
                            <option value="{{ $promotor->id }}" {{ old('promotor_id', $producto->promotor_id) == $promotor->id ? 'selected' : '' }}>
